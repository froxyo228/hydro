# ========== EconomicSystem.gd ==========
# Экономическая система - управляет контрактами, доходами и расходами
extends Node
...
## Класс энергетического контракта
class PowerContract:
	var id: String
...
func _ready():
	...
	add_child(timer)
	print("[SYSTEM] EconomicSystem инициализирован")

## Добавить новый энергетический контракт
func add_contract(power_req: float, price: float, duration: float) -> String:
	...
	print("[RESOURCE] Новый контракт: %s (%.1f МВт, %.0f руб/МВтч)" % [contract.id, power_req, price])
	return contract.id
