# ========== SaveSystem.gd ==========
# Система сохранения - управляет сохранением и загрузкой игрового прогресса
extends Node
...
func _ready():
	...
	print("[SYSTEM] SaveSystem инициализирован")

## Сохранить текущее состояние игры
func save_game() -> bool:
	var save_file = FileAccess.open(SAVE_FILE, FileAccess.WRITE)
	if save_file == null:
		print("[SAVE] Ошибка создания файла сохранения")
		return false
	
	var save_data = {}
	
	# Сохраняем состояние GameManager
	if GameManager:
	...
	# Сохраняем состояние RiverSystem
	...
	save_file.close()
	print("[SAVE] Игра сохранена")
	return true

## Загрузить сохраненное состояние игры
func load_game() -> bool:
	if not FileAccess.file_exists(SAVE_FILE):
		print("[SAVE] Файл сохранения не найден")
		return false
	
	var save_file = FileAccess.open(SAVE_FILE, FileAccess.READ)
	if save_file == null:
		print("[SAVE] Ошибка чтения файла сохранения")
		return false
	...
	if parse_result != OK:
		print("[SAVE] Ошибка парсинга файла сохранения")
		return false
	
	var save_data = json.data
	
	# Загружаем состояние GameManager
	...
	if save_data.has("build_zones"):
		...
		for i in range(min(zones_data.size(), zones.size())):
			...
			if zone.has_method("update_zone_visual"):
				zone.update_zone_visual()
	
	# Загружаем состояние RiverSystem
