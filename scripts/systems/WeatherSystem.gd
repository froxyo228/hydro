# ========== WeatherSystem.gd ==========
# Система погоды - управляет погодными условиями и их влиянием на реку
extends Node
...
func _ready():
	...
	add_child(timer)
	print("[SYSTEM] WeatherSystem инициализирован")

## Изменить погодные условия
func _change_weather():
	...
	print("[WEATHER] Погода изменилась: %s (интенсивность: %.1f)" % [get_weather_string(), weather_intensity])
	
	var river_system = get_tree().get_first_node_in_group("river_system")
	if river_system:
		apply_weather_effects(river_system)

## Применить влияние погоды на систему реки
...
## Получить строковое представление текущей погоды
